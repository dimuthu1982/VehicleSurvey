...
public class SpeedCountingStatistics extends CountingStatistics {
	...
	public String toString() {
		...
		if(upStreamSpeed > 0) {
			message = String.format("Upstream speed: %d KM/H, Total vehicles traveld: %d, Time Period: %s", calculateUpStreamAverage(), super.getUpStreamCount(),formatTimeStatistics());
		}
		
		if(downStreamSpeed > 0) {
			message = message.concat("\n").concat(String.format("Downstream speed: %d, Total vehicles traveld: %d, Time Period: %s", calculatDownStreamAverage(), super.getDownStreamCount(),formatTimeStatistics()));
		}
